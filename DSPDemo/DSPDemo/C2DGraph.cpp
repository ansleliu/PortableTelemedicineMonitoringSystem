...
{
	//初始化控件属性
	m_crBackColor = RGB(0,0,0);      //背景色设定
	m_crGridColor = RGB(0,255,255);  //栅格线颜色设定
	m_crLineColor = RGB(0,255,0);    //曲线颜色设定
	m_crTextColor = RGB(255,0,255);  //轴示文本设定	

	m_strXCaption = _T("时间");      //X轴示文字设定
	m_strYCaption = _T("Y X S");     //Y轴示文字设定

	m_dXMaxValue = X_MAX;            //X轴最大值
	m_dXMinValue = 0;                //X轴最小值
	m_dYMaxValue = Y_MAX;            //Y轴最大值
	m_dYMinValue = 0;                //Y轴最小值
...
void C2DGraph::InvalidateCtrl()
{
	...
	//创建Y轴字体
	yFont.CreateFont (8, 0, 900, 0, 400,
...
//在曲线末尾添加一个点
void C2DGraph::AppendPoint(double dwPointY)
{
	//在队列中添加1个点
